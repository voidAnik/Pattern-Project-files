...
def KNN_Classifier_binary(X, y, indep=None, fold=5, n_neighbors=3, out='KNN_output'):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(usage="it's usage tip.",
                                     description="training KNN model.")
    ...
    parser.add_argument("--k", type=int, default=3, help="the K nearest neighbour value. default: 3")
    parser.add_argument("--fold", type=int, default=5,
                        help="n-fold cross validation mode (default 5-fold cross-validation, 1 means jack-knife cross-validation)")
    parser.add_argument("--out", default="KNN_output", help="set prefix for output score file")
    ...
    para_info, cv_res, ind_res = KNN_Classifier_binary(X, y, indep=independent, fold=args.fold, n_neighbors=args.k,
                                                       out=args.out)
